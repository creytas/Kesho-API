...
const select_user_attendances = `SELECT DISTINCT users.id AS "id_user", prenom_user, nom_user, postnom_user, email, sexe_user, is_admin, statut,(SELECT attendances.attendance_state FROM users, attendances WHERE users.id=attendances.user_id AND attendances.createdAt IN (SELECT MIN(attendances.createdAt) FROM attendances)) AS "attendance" FROM attendances, users`;
