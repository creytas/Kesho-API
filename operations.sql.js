...
const select_user_attendances = `SELECT users.id, users.id_user, users.prenom_user, users.nom_user, users.postnom_user, users.email, users.sexe_user, users.statut, users.is_admin, attendances.createdAt, attendances.attendance_state FROM users, attendances WHERE users.id=attendances.user_id AND attendances.createdAt IN (SELECT MIN(attendances.createdAt) FROM attendances, users WHERE DATE_FORMAT(attendances.createdAt, "%Y-%m-%d")=DATE_FORMAT(CURRENT_DATE,"%Y-%m-%d") AND attendances.user_id=users.id)`;
